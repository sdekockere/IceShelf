...
This should be the file with the ".reas" extension used by CoREAS during the CORSIKA simulation. Please refere to the CoREAS manual for more information.
#### 7. The list file used during the CORSIKA (CoREAS) simulation
This should be the file with the ".list" extension used by CoREAS during the CORSIKA simulation, listing the observer positions used during the simulation for the radio calculations. Please refere to the CoREAS manual for more information.
...
The direct ray solution is defined as the solution with the shortest ray path between emitter and observer.
...
The indirect ray solution is defined as the solution with the longest ray path between emitter and observer.
#### 5. The "_SA.txt" file
...
Note that the sudden appearance contributions are also included in the "_dir.txt" and "_indir.txt" files. To get the total emission, simply add up the contributions from the "_dir.txt" and "_indir.txt" files, and do NOT add the contributions from the "_SA.txt" file as well. If you do, you're essentially counting the "_SA.txt" contributions twice.
...
##### ```sPeriod```
Defines the sampling rate of the observers (unit time), i.e. it is the $\Delta t$ in the endpoint formula.
##### ```traceLength```
...
##### ```approxThreshold```
Defines the treshold on the doppler factor (or boosting factor) $1 - n \vec{\beta}^* \cdot \hat{r}$. When the factor falls below (<) this threshold, either for the start point OR the end point of a step, the endpoint algorithm essentially used the ZHS algorithm instead. The start- en endpoint are replaced by the middle point of the step, and the finite time resoltion of the observer is taken into account explicitly.
